...
█""")
#-----------------------( INFO )-----------------------#
info=(f"""{xp} AUTHOR    {xpxxx} {green}BILAL BALOCH 
{xp} FACEBOOK  {xpxxx} {green}BILAL BALOCH 
{xp} TYPE      {xpxxx} {green}AUTO CREATE FACEBOOK
{xp} OWNER    {xpxxx} {green}BILAL BALOCH 
...
def __AUTOX__():
    ...
    __ERRORLOGO__()
    print(f"{xp1} MALE NAME UNITED KINGDOM  ")
    ...
    for _ in range(int(__NUM__)):
        try:
            ...
            sys.stdout.write(f'\r {white}[{green}BILAL-CREATE{white}]-{white}[{green}OK:-%s{white}] '%(len(oks)));sys.stdout.flush()
            sys.stdout.flush()
            ...
            if "c_user" in login_coki:
                ...
                if show_details=='y':
                    ...
                    print(f'\r{green} [BILAL-OK] '+uid+' | '+pww+'\033[1;97m')
                open('/sdcard/BILAL-AUTO/AUTO/BILAL-AUTO-OK.txt', 'a').write(uid+'|'+pww+'|'+coki+'\n')
                oks.append(uid)
